...
import glass.brew.napkin.utils.RequestManager;
...
public class ApiBase {
    ...
    protected String getResponseFromRemote(String api_url, String method, String type) {
         ...
         */
        if (type.equals("httpa")) {
            rm.addHeader("X-Auth-Token", PreferenceManager.getSessionKeyInPrefs(mContext));
        }
         ...
         */
        if (isFailCommunication(response)) {
            return null;
        }
    ...
    private boolean isFailCommunication(String response) {
        ...
        try {
            ...
            if (message.equals("Invalid \"session key\".") || message.equals("No such auch.") || message.equals("Expired \"session key\".")) {
                this.request_code = Argument.REQUEST_CODE_FAIL_LOGIN;
                PreferenceManager.put(mContext, Argument.PREFS_LOG_ON, false);
                return true;
            }
